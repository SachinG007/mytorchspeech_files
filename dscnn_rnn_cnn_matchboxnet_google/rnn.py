...
def gen_nonlinearity(A, nonlinearity):
    ...
    if nonlinearity == "tanh":
        ...
        return torch.max(torch.min(A, torch.ones_like(A)),
                         -1.0 * torch.ones_like(A))
...
class RNNCell(nn.Module):
    def __init__(self,
        ...
        self._num_weight_matrices = [
            self._num_W_matrices, self._num_U_matrices, self._num_biases
        ]
        self._wRank = wRank
        self._uRank = uRank
    ...
    def copy_previous_UW(self):
        ...
        for i in range(num_mats):
            self.oldmats[i] = torch.FloatTensor(mats[i].detach().clone().to(
                mats[i].device))
...
class FastGRNNCell(RNNCell):
    ...
    def __init__(self,
        ...
        super(FastGRNNCell,
              self).__init__(input_size, hidden_size, gate_nonlinearity,
                             update_nonlinearity, 1, 1, 2, wRank, uRank,
                             wSparsity, uSparsity)
    ...
    def forward(self, input, state):
        if self._wRank is None:
            ...
            wComp = torch.matmul(
                torch.matmul(input, torch.transpose(self.W1, 0, 1)),
                torch.transpose(self.W2, 0, 1))

        if self._uRank is None:
            uComp = torch.matmul(state, torch.transpose(self.U, 0, 1))
        else:
            uComp = torch.matmul(
                torch.matmul(state, torch.transpose(self.U1, 0, 1)),
                torch.transpose(self.U2, 0, 1))
...
class FastGRNNCUDACell(RNNCell):
    ...
    def __init__(self,
        ...
        super(FastGRNNCUDACell,
              self).__init__(input_size, hidden_size, gate_non_linearity,
                             update_nonlinearity, 1, 1, 2, wRank, uRank,
                             wSparsity, uSparsity)
        ...
        if wRank is None:
            self.W = nn.Parameter(
                0.1 * torch.randn([hidden_size, input_size], self.device))
            self.W1 = torch.empty(0)
            self.W2 = torch.empty(0)
        else:
            self.W = torch.empty(0)
            self.W1 = nn.Parameter(
                0.1 * torch.randn([wRank, input_size], self.device))
            self.W2 = nn.Parameter(
                0.1 * torch.randn([hidden_size, wRank], self.device))

        if uRank is None:
            self.U = nn.Parameter(
                0.1 * torch.randn([hidden_size, hidden_size], self.device))
            self.U1 = torch.empty(0)
            self.U2 = torch.empty(0)
        else:
            self.U = torch.empty(0)
            self.U1 = nn.Parameter(
                0.1 * torch.randn([uRank, hidden_size], self.device))
            self.U2 = nn.Parameter(
                0.1 * torch.randn([hidden_size, uRank], self.device))

        self._gate_non_linearity = NON_LINEARITY[gate_nonlinearity]

        self.bias_gate = nn.Parameter(torch.ones([1, hidden_size],
                                                 self.device))
        self.bias_update = nn.Parameter(
            torch.ones([1, hidden_size], self.device))
        self.zeta = nn.Parameter(self._zetaInit *
                                 torch.ones([1, 1], self.device))
    ...
    def forward(self, input, state):
        ...
        return FastGRNNFunction.apply(input, self.bias_gate, self.bias_update,
                                      self.zeta, self.nu, state, self.W,
                                      self.U, self.W1, self.W2, self.U1,
                                      self.U2, self._gate_non_linearity)
...
class FastRNNCell(RNNCell):
    ...
    def __init__(self,
        ...
        super(FastRNNCell, self).__init__(input_size, hidden_size, None,
                                          update_nonlinearity, 1, 1, 1, wRank,
                                          uRank, wSparsity, uSparsity)
...
class LSTMLRCell(RNNCell):
    ...
    def __init__(self,
        ...
        super(LSTMLRCell,
              self).__init__(input_size, hidden_size, gate_nonlinearity,
                             update_nonlinearity, 4, 4, 4, wRank, uRank,
                             wSparsity, uSparsity)
        ...
        if wRank is None:
            self.W1 = nn.Parameter(0.1 *
                                   torch.randn([input_size, hidden_size]))
            self.W2 = nn.Parameter(0.1 *
                                   torch.randn([input_size, hidden_size]))
            self.W3 = nn.Parameter(0.1 *
                                   torch.randn([input_size, hidden_size]))
            self.W4 = nn.Parameter(0.1 *
                                   torch.randn([input_size, hidden_size]))
        else:
        ...
        if uRank is None:
            self.U1 = nn.Parameter(0.1 *
                                   torch.randn([hidden_size, hidden_size]))
            self.U2 = nn.Parameter(0.1 *
                                   torch.randn([hidden_size, hidden_size]))
            self.U3 = nn.Parameter(0.1 *
                                   torch.randn([hidden_size, hidden_size]))
            self.U4 = nn.Parameter(0.1 *
                                   torch.randn([hidden_size, hidden_size]))
        else:
    ...
    def forward(self, input, hiddenStates):
        ...
        if self._wRank is None:
            ...
            wComp4 = torch.matmul(torch.matmul(input, self.W), self.W4)
        ...
        if self._uRank is None:
            ...
            uComp1 = torch.matmul(torch.matmul(h, self.U), self.U1)
            uComp2 = torch.matmul(torch.matmul(h, self.U), self.U2)
            uComp3 = torch.matmul(torch.matmul(h, self.U), self.U3)
            uComp4 = torch.matmul(torch.matmul(h, self.U), self.U4)
        pre_comp1 = wComp1 + uComp1
        pre_comp2 = wComp2 + uComp2
        pre_comp3 = wComp3 + uComp3
        pre_comp4 = wComp4 + uComp4

        i = gen_nonlinearity(pre_comp1 + self.bias_i, self._gate_nonlinearity)
        f = gen_nonlinearity(pre_comp2 + self.bias_f, self._gate_nonlinearity)
        o = gen_nonlinearity(pre_comp4 + self.bias_o, self._gate_nonlinearity)
...
class GRULRCell(RNNCell):
    ...
    def __init__(self,
        ...
        super(GRULRCell,
              self).__init__(input_size, hidden_size, gate_nonlinearity,
                             update_nonlinearity, 3, 3, 3, wRank, uRank,
                             wSparsity, uSparsity)
...
class UGRNNLRCell(RNNCell):
    ...
    def __init__(self,
        ...
        super(UGRNNLRCell,
              self).__init__(input_size, hidden_size, gate_nonlinearity,
                             update_nonlinearity, 2, 2, 2, wRank, uRank,
                             wSparsity, uSparsity)
...
class UGRNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.cell = UGRNNLRCell(input_size,
                                hidden_size,
                                gate_nonlinearity=gate_nonlinearity,
                                update_nonlinearity=update_nonlinearity,
                                wRank=wRank,
...
class FastRNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.cell = FastRNNCell(input_size,
                                hidden_size,
                                gate_nonlinearity=gate_nonlinearity,
                                update_nonlinearity=update_nonlinearity,
                                wRank=wRank,
...
class FastGRNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.cell = FastGRNNCell(input_size,
                                 hidden_size,
                                 gate_nonlinearity=gate_nonlinearity,
                                 update_nonlinearity=update_nonlinearity,
                                 wRank=wRank,
...
class FastGRNNCUDA(nn.Module):
    ...
    def __init__(self,
        ...
        if wRank is None:
            self.W = nn.Parameter(
                0.1 *
                torch.randn([hidden_size, input_size], device=self.device))
            self.W1 = torch.empty(0)
            self.W2 = torch.empty(0)
        else:
            self.W = torch.empty(0)
            self.W1 = nn.Parameter(
                0.1 * torch.randn([wRank, input_size], device=self.device))
            self.W2 = nn.Parameter(
                0.1 * torch.randn([hidden_size, wRank], device=self.device))
        ...
        if uRank is None:
            ...
            self.U = torch.empty(0)
            self.U1 = nn.Parameter(
                0.1 * torch.randn([uRank, hidden_size], device=self.device))
            self.U2 = nn.Parameter(
                0.1 * torch.randn([hidden_size, uRank], device=self.device))

        self._gate_non_linearity = NON_LINEARITY[gate_nonlinearity]

        self.bias_gate = nn.Parameter(
            torch.ones([1, hidden_size], device=self.device))
        self.bias_update = nn.Parameter(
            torch.ones([1, hidden_size], device=self.device))
        self.zeta = nn.Parameter(self._zetaInit *
                                 torch.ones([1, 1], device=self.device))
        self.nu = nn.Parameter(self._nuInit *
                               torch.ones([1, 1], device=self.device))
    ...
    def forward(self, input, hiddenState=None, cell_state=None):
        ...
        if hiddenState is None:
            hiddenState = torch.zeros([input.shape[1],
                                       self._hidden_size]).to(self.device)
        if not hiddenState.is_cuda:
            hiddenState = hiddenState.to(self.device)
        result = FastGRNNUnrollFunction.apply(input, self.bias_gate,
                                              self.bias_update, self.zeta,
                                              self.nu, hiddenState, self.W,
                                              self.U, self.W1, self.W2,
                                              self.U1, self.U2,
...
class SRNN2(nn.Module):
    def __init__(self,
                 ...
                 hiddenDim1,
                 cellType,
                 dropoutProbability0=None,
                 dropoutProbability1=None,
                 **cellArgs):
        '''
        ...
        self.cellArgs = {'batch_first': False}
        ...
        self.rnn0 = self.rnnClass(input_size=inputDim,
                                  hidden_size=hiddenDim0,
                                  **self.cellArgs)
        self.rnn1 = self.rnnClass(input_size=hiddenDim0,
    ...
    def getBrickedData(self, x, brickSize):
        ...
        x_bricked_batched = torch.reshape(
            x_bricked_batched, (numBricks, brickSize, batchSize, featureDim))
    ...
    def forward(self, x, brickSize):
        ...
        x_bricks = torch.reshape(
            x_bricks, [oldShape[0], oldShape[1] * oldShape[2], oldShape[3]])
        # x bricks: [brickSize, numBricks * batchSize, featureDim]
...
class FastGRNNFunction(Function):
    @staticmethod
    def forward(ctx, input, bias_gate, bias_update, zeta, nu, old_h, w, u, w1,
                w2, u1, u2, gate_non_linearity):
        outputs = fastgrnn_cuda.forward(input, w, u, bias_gate, bias_update,
                                        zeta, nu, old_h, gate_non_linearity,
                                        w1, w2, u1, u2)
        new_h = outputs[0]
        variables = [input, old_h, zeta, nu, w, u
                     ] + outputs[1:] + [w1, w2, u1, u2]
    ...
    def backward(ctx, grad_h):
        outputs = fastgrnn_cuda.backward(grad_h.contiguous(),
                                         *ctx.saved_variables,
                                         ctx.non_linearity)
...
class FastGRNNUnrollFunction(Function):
    ...
    def forward(ctx, input, bias_gate, bias_update, zeta, nu, old_h, w, u, w1,
                w2, u1, u2, gate_non_linearity):
        outputs = fastgrnn_cuda.forward_unroll(input, w, u, bias_gate,
                                               bias_update, zeta, nu, old_h,
                                               gate_non_linearity, w1, w2, u1,
                                               u2)
        hidden_states = outputs[0]
        variables = [input, hidden_states, zeta, nu, w, u
                     ] + outputs[1:] + [old_h, w1, w2, u1, u2]
        ctx.save_for_backward(*variables)
    ...
    @staticmethod
    def backward(ctx, grad_h):
        outputs = fastgrnn_cuda.backward_unroll(grad_h.contiguous(),
                                                *ctx.saved_variables,
                                                ctx.gate_non_linearity)
        return tuple(outputs + [None])
