...
def main():
    ...
    if args.words == 'all':
        ...
        if args.version == '2' or args.version == 'v2':
            args.words = ["happy", "four", "up", "visual", "follow", "nine", "two", "left", 
                "wow", "five", "no", "seven", "down", "sheila", "house", "cat", "learn", "on", 
                "backward", "go", "off", "stop", "tree", "three", "eight", "six", "zero", 
                "dog", "bed", "right", "yes", "bird", "forward", "one", "marvin"
            ]
        if args.tts:
    ...
    for i, config in enumerate(models_config):
        ...
        MODEL_NAME = 'RNNCNN' + '-LR{}-rank{}-channel{}-inpsize{}'.format(0.001, config.rank, config.cnn_channels,config.inp_size)
        ...
        classifier = kwscnn.Binary_Classification_Block_withconv(config.inp_size,
                                                        100,
                                                        1,
                                                        device,
